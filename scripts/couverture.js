...
const comptes = {
  ...
  visites:     { certain: 0, estimation: 0, inconnu: 0 },
  pages_vues:  { certain: 0, estimation: 0, inconnu: 0 },
...
for (const id of ids) {
  ...
  const visV = p.visites.valeur;
  comptes.visites[p.visites.couche === "certain" ? "certain" : visV == null || Number.isNaN(visV) ? "inconnu" : "estimation"]++;
  const pvV = p.pagesVues.valeur;
  comptes.pages_vues[p.pagesVues.couche === "certain" ? "certain" : pvV == null || Number.isNaN(pvV) ? "inconnu" : "estimation"]++;
...
console.log(`  audience visites  : ${comptes.visites.certain} certains (ACPM), ${comptes.visites.estimation} estimés, ${comptes.visites.inconnu} inconnus   [clé d'audience par défaut]`);
console.log(`  audience pages_vues: ${comptes.pages_vues.certain} certains (ACPM), ${comptes.pages_vues.estimation} estimés, ${comptes.pages_vues.inconnu} inconnus   [clé alternative ; estim = visites × pages_par_visite famille]`);
...
console.log(`  visites (ACPM)   : ${nb("visites")} titres   (clé d'audience visites)`);
console.log(`  pages_vues (ACPM): ${nb("pages_vues")} titres   (clé d'audience pages vues)`);
console.log(`  marche           : ${Object.keys(d.marche).length} agrégats`);

const trous = comptes.abonnes_num.inconnu + comptes.arpu.inconnu + comptes.visites.inconnu + comptes.pages_vues.inconnu + comptes.indice_inference.inconnu;
...
console.log(`  visites ACPM      : ${nb("visites")}/${N} sourcées — tableau complet (231 sites) = Classement Unifié ACPM, à compléter par PR.`);
console.log(`  pages_vues ACPM   : ${nb("pages_vues")}/${N} sourcées — même classement (colonne Pages Vues Totales), à compléter par PR.`);
console.log(`  diffusion ACPM    : ${nb("diffusion_payee")}/${N} sourcées — reste des classements PQR/magazines à compléter par PR.`);
console.log("NON SOURÇABLES PUBLIQUEMENT (resteront estimées) : visiteurs uniques & temps de lecture (Médiamétrie payant), inférences IA (non publiées), ARPU (pas de prix catalogue fiable).");
